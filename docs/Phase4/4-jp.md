...
### <!-- fit --> なぜ Git がチーム開発において必要なのか :sunglasses:
...
# <!-- fit --> 少しは **ブランチ** というものが身近になりましたかね :heart:
...
1. :black_square_button: 作業ディレクトリでファイルの追加を行う
...
0. :white_check_mark: ~~本日新たに `clone` したディレクトリを VS Code に追加~~
1. :black_square_button: 作業ディレクトリでファイルの追加を行う
2. :black_square_button: 変更内容を「ローカルリポジトリ」へ反映
3. :black_square_button: 「ローカルリポジトリ」の内容を「リモートリポジトリ」へ反映
...
### 1. 作業ディレクトリでファイルの追加を行う

VS Code で下記のディレクトリを選択してください。

```bash
titech-2020-day4/titech-git-day4/greeting
```
...
:notebook: 同じ並びにある `template.txt` の中身をコピーしても OK 🙆‍♀️
...
```bash
git add titech-git-day4/greeting/{your-name}.txt
```
...
「リモートリポジトリに対して `feature/{your-name}` ブランチの内容を push する」
...
![w:300px](images/github_octocat.png)
...
上記が終わったら `titech-2020-day4/titech-git-day4/greeting` ディレクトリの中を VS Code で見てみましょう！
...
（座席はそのままで構いません）
（もしどうしても 1 人になりそう、という方がいらっしゃったら、GW のメンバがペアになるのでお声がけください）
...
* **Y [やったこと]**：自分の経験として取り組んだことを振り返る
  * 考える：10min
* **W [わかったこと]**：そこから学んだ経験を考える
  * 考える：10min / どなたか発表してくれる方〜〜 :information_desk_person::information_desk_person:
* **T [次にやること]**：その経験を踏まえて、継続・改善するためにやることを考える
  * 考える：10min / どなたか発表してくれる方〜〜 :raising_hand::raising_hand:
...
#### ここまでの Y と W を発表したい方いらっしゃいますかー🙋‍♀️ ？
...
# <!-- fit --> 以上で『チーム開発に必要なこと：コミュニケーション編』は終了です！
...
メール 💌 なり、zoom なり、合宿の時にでも熱く語りましょう :beer::smile:
